...
// define the structure of 1-message/3-recipients ciphertext
struct Triple_Twisted_ElGamal_CT
{
    EC_POINT *X1; // X = pk1^r
    EC_POINT *X2; // X = pk2^r
    EC_POINT *X3; // X = pk3^r 
...
void Triple_Twisted_ElGamal_CT_new(Triple_Twisted_ElGamal_CT &CT)
{
    CT.X1 = EC_POINT_new(group);
    CT.X2 = EC_POINT_new(group);
    CT.X3 = EC_POINT_new(group);  
    CT.Y = EC_POINT_new(group);
}

void Triple_Twisted_ElGamal_CT_free(Triple_Twisted_ElGamal_CT &CT)
{
    EC_POINT_free(CT.X1);
    EC_POINT_free(CT.X2); 
    EC_POINT_free(CT.X3); 
...
void Triple_Twisted_ElGamal_CT_print(Triple_Twisted_ElGamal_CT &CT)
{
    ECP_print(CT.X1, "CT.X1");
    ECP_print(CT.X2, "CT.X2");
    ECP_print(CT.X3, "CT.X3");
    ECP_print(CT.Y, "CT.Y");
} 
...
void Triple_Twisted_ElGamal_CT_serialize(Triple_Twisted_ElGamal_CT &CT, ofstream& fout)
{
    ECP_serialize(CT.X1, fout); 
    ECP_serialize(CT.X2, fout);
    ECP_serialize(CT.X3, fout);
    ECP_serialize(CT.Y,  fout); 
} 

void Triple_Twisted_ElGamal_CT_deserialize(Triple_Twisted_ElGamal_CT &CT, ifstream& fin)
{
    ECP_deserialize(CT.X1, fin); 
    ECP_deserialize(CT.X2, fin); 
    ECP_deserialize(CT.X3, fin); 
    ECP_deserialize(CT.Y, fin); 
}
...
/* Encryption algorithm (1-message/3-recipients) with given random coins
output X1 = pk1^r, X2 = pk2^r, X3 = pk3^r, Y = g^r h^m
Here we make the randomness explict for the ease of generating the ZKP */
void Triple_Twisted_ElGamal_Enc(Twisted_ElGamal_PP &pp, 
                            EC_POINT* &pk1, EC_POINT* &pk2, EC_POINT* &pk3, 
                            BIGNUM* &m, BIGNUM* &r, Triple_Twisted_ElGamal_CT &CT)
{ 
    EC_POINT_mul(group, CT.X1, NULL, pk1, r, bn_ctx); // CT_new.X1 = pk1^r
    EC_POINT_mul(group, CT.X2, NULL, pk2, r, bn_ctx); // CT_new.X2 = pk2^r
    EC_POINT_mul(group, CT.X3, NULL, pk3, r, bn_ctx); // CT_new.X3 = pk3^r
    EC_POINT_mul(group, CT.Y, r, pp.h, m, bn_ctx); // Y = g^r h^m
   
    #ifdef DEBUG
        cout << "1-message/3-recipients twisted ElGamal encryption finishes >>>"<< endl;
        Triple_Twisted_ElGamal_CT_print(CT); 
    #endif
}
